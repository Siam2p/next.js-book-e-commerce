  ...
  return (
    <>
      {links.map((link:{name: string, href: string, icon: React.ComponentType<React.SVGProps<SVGSVGElement>>}) => {
        ...
        return(
          <Link key={link.name} href={link.href} className={`flex h-[48px] grow items-center justify-center gap-2 rounded-md bg-gray-50 p-3 text-sm font-medium hover:bg-sky-100 hover:text-purple-600 md:flex-none md:justify-start md:p-2 md:px-3 ${link.name === 'Cart' && cartData.length <= 0 && 'hidden'}`}>
