...
export default function OrderDetails() {
  ...
  const priceToBuy: number = booksToBuy.reduce((total: number, book: Book) => total + (book.sellPrice * book.quantity!), 0);
  const totalBuyBook = booksToBuy.reduce((total, item)=> total + (item.quantity ?? 0), 0 );
  
  const booksToRent: Book[] = cartData.filter((book: Book) => book.type === 'rent');
  const priceToRent: number = booksToRent.reduce((total: number, book: Book) => total + (book.rentPrice * book.quantity!), 0);
  const totalRentBook = booksToRent.reduce((total, item)=> total + (item.quantity ?? 0), 0 );

  const deliveryCharge: number = deliveryBase[(priceToBuy < 1000 ? totalBuyBook : 0) + (priceToRent < 300 ? totalRentBook : 0)];
  ...
  return (
    <div className='w-full bg-white rounded px-8 pt-6 pb-8 mb-4'>
      ...
      (<ul>
        {booksToBuy.length > 0 && <li>Buying {totalBuyBook} books for <CurrencyBangladeshiIcon className="w-5 h-5 inline"/> {priceToBuy} BDT</li>}
        {booksToRent.length > 0 && <li>Renting {totalRentBook} books for <CurrencyBangladeshiIcon className="w-5 h-5 inline"/> {priceToRent} BDT</li>}
